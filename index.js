...
function filterProductsByPriceRange(products, min, max) {
  if(!products.length) throw "Array is empty."
  if(typeof max !== "number" || typeof min !== "number") throw "Either `min` or `max` is not a number.";
  if(max<=0 || min<0 || min>max) throw "Inappropriate max or min values";
  
  const result = [];
  for (let product of products) {
    if(!product.priceInCents) throw "A product of the `products` array does not have a `priceInCents` key";
